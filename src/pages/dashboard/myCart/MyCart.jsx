  ...
  const handleDelete = item =>{
    Swal.fire({
        ...
        if (result.isConfirmed) {
         fetch(`https://art-school-server-nine.vercel.app/carts/${item._id}`, {
            method: 'DELETE'
         })
         .then(res=> res.json())
         .then(data=>{
  ...
  return (
    <div className="w-full h-full bg-gradient-to-r from-purple-500 to-violet-400">
      ...
      <div className="flex justify-evenly rounded-l-none text-white mt-24 mb-12">
        ...
        <Link to="/dashboard/payment" className="btn btn-sm">Pay Now</Link>
      </div>
          ...
          <tbody>
            {cart.map((row, index) => (
              ...
              >
                ...
                <td className=""><span className="end ">
                    ${row.price}
                  </span></td>
                <td>
